...
공용 폴백: `app/error.tsx`(세그먼트 에러), `app/global-error.tsx`(루트 레이아웃까지 죽은 경우), `app/not-found.tsx`(404).
...
- `getBestProducts()` 등은 예전엔 `catch` 후 `[]` / `{}`를 반환해서 **인프라가 막혀도 에러가 아니라
  "상품 없음"으로 보였다.** 지금은 예외를 그대로 던진다(23e61a4). 즉 netpol/서비스가 막히면 메인이
  빈 화면이 아니라 500이 된다. **이 동작을 `catch`로 되돌리지 말 것** — #41이 의도적으로 없앤 것이다.
  화면은 `app/error.tsx`가 받아 "일시적인 오류 + 재시도" 안내로 바꾼다(#57). 응답 코드는 그대로
  500이라 모니터링에서 장애는 여전히 보인다.
- **`app/page.tsx`의 `export const dynamic = "force-dynamic"`을 지우지 말 것**(#53). 이게 없으면
  Next가 이 페이지를 빌드 타임에 프리렌더하려 들고, GitHub 호스티드 러너에서는 위 내부 주소가
  해석되지 않아 **Docker 빌드가 통째로 실패한다**(= 이 저장소의 모든 배포가 멈춘다).
  예외를 삼키도록 되돌리면 빌드는 살지만 이번엔 "빈 쇼핑몰" HTML이 이미지에 구워진다(#41).
  빌드 타임에 내부 API를 부르지 않는 것이 두 증상의 유일한 공통 해법이다.
  `next: { revalidate }`가 붙은 fetch는 `force-dynamic` 아래에서도 캐시되므로
  (Next 15는 캐시 설정이 **없는** fetch만 no-store로 덮는다) 상위 API 호출은 매 요청이 아니다.
